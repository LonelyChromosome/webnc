...
function sendFrontendFile(res, relativePath, contentType) {
  sendFile(res, path.join(FRONTEND_DIR, relativePath), contentType);
}
...
function renderView(res, viewName, data = {}) {
  const filePath = path.join(VIEWS_DIR, viewName);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.url === "/" || req.url === "/intro") {
      sendFile(res, path.join(PAGES_DIR, "intro.html"));
      return;
    }

    if (req.url === "/contact" || req.url === "/about") {
      sendFile(res, path.join(PAGES_DIR, "contact.html"));
      return;
    }
    ...
    if (req.url === "/js/demo.js" || req.url === "/demo.js") {
      sendFrontendFile(
        res,
        path.join("js", "demo.js"),
